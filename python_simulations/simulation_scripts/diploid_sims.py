...
def random_mating(g):
    #takes in 2-element list of gamete frequencies "g" and returns a 1D convolution to produce genotype frequencies
#    print(g)
    return signal.convolve(g, g, method='direct')

def selection(G, fitness):
    #takes in 3-element list "G" and 3-element list "fitness"
...
def meiosis(G, seg):
    #takes in 3-element list of genotype frequencies "G" and 3-element list of segregation patterns "seg"
    #each segregation pattern in "seg" is the gamete frequencies produced from each genotype
    g0 = [seg[0][i] * G[0] for i in range(2)]
    g1 = [seg[1][i] * G[1] for i in range(2)] 
    g2 = [seg[2][i] * G[2] for i in range(2)]
    g = [sum(x) for x in zip(g0, g1, g2)]
    return g
...
def mutation(g, mu, nu):
    ...
    g0 = g[0]*(1.0-mu) + g[1]*nu
    g1 = g[0]*mu + g[1]*(1.0-nu)
    return [g0, g1]
...
def main():
    ...
    header = 'Generation,s,h,mu,nu,g0,g1,G0,G1,G2'

    if inargs.G:
        gams = [np.nan, np.nan]
        genos = [inargs.G0, inargs.G1, inargs.G2]
        if abs(sum(genos) - 1.0) > 1e-6:
            raise Exception("Initial Genotype Frequencies must sum to 1.0, not ", sum(genos)) 

    if inargs.g:
        gams = [inargs.g0, inargs.g1]
        if abs(sum(gams) - 1.0) > 1e-6:
            raise Exception("Initial Gamete Frequencies must sum to 1.0, not ", sum(gams))
        genos = random_mating(gams)

    fitness = set_fitness(inargs.s, inargs.h1)

    segs = set_seg()

    gen_stats = np.array([[x for y in [[0, inargs.s, inargs.h1, inargs.mu, inargs.nu], gams, genos] for x in y]])
    gams = generation(genos, segs, fitness, inargs.mu, inargs.nu)
    genos = random_mating(gams)

    for i in range(1, inargs.N):
        add_stack = [x for y in [[i, inargs.s, inargs.h1, inargs.mu, inargs.nu], gams, genos] for x in y]
#        print(add_stack)
    ...
    np.savetxt(inargs.o, gen_stats, delimiter=',', header = header, fmt = ['%g', '%.3e', '%.3e', '%.3e', '%.3e', '%.18e', '%.18e', '%.18e', '%.18e', '%.18e'])
